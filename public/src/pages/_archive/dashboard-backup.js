...
<div class="header-actions">
    ...
    initializeComponents() {
        ...
        try {
            ...
            if (timerContainer) {
                ...
                // Make timer accessible through multiple paths for audio player
                if (window.app && window.app.currentPage) {
                    window.app.currentPage.timer = timer;
                    window.app.currentPage.sharedTimer = timer;
                }
    ...
    async initializeCloudSync() {
        ...
        window.addEventListener('practiceDataUpdated', () => {
            this.loadRecentSessions();
            this.loadQuickStats();
        });
    }
...
// Initialize shared components across tabs
    initializeSharedComponents() {
        ...
        // Make timer globally accessible for audio sync
        if (this.components.timer) {
            // Store reference on window for easy access
            window.currentTimer = this.components.timer;
            console.log('Timer made globally accessible as window.currentTimer');
        }
    }
