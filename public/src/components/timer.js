...
export class Timer {
    ...
    `;
        ...
        // Handle cancel button with custom confirmation dialog
        modal.querySelector('.btn-secondary').addEventListener('click', () => {
            // Check if there's elapsed time (practice session in progress)
            if (this.elapsedTime > 0) {
                ...
                confirmDialog.innerHTML = `
                    ...
                    ">
                        <h3 style="margin-bottom: 16px;">Active Practice Session</h3>
                        <p style="margin-bottom: 24px; color: var(--text-secondary);">
                            You have an active practice session. Do you want to save it before closing?
                        </p>
                        <div style="display: flex; gap: 12px; justify-content: center;">
                            <button class="btn btn-primary" id="saveSessionBtn">Save Session</button>
                            <button class="btn btn-secondary" id="dontSaveBtn">Don't Save</button>
                        </div>
                    </div>
                `;
        ...
        // Handle ESC key to close modal with confirmation
        const handleEscKey = (e) => {
            if (e.key === 'Escape' || e.key === 'Esc') {
                // Trigger the cancel button click to show confirmation if needed
        ...
        // Also remove the ESC event listener when modal is closed via form submission
        const originalSubmitHandler = modal.querySelector('#practiceLogForm').onsubmit;
