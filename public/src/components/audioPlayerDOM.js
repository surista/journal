...
function createAudioSourceSection() {
    ...
    fileTab.style.cssText = 'flex: 1; padding: 10px; background: var(--primary); color: white; border: none; border-radius: 8px; cursor: pointer; transition: all 0.2s;';
    ...
    youtubeTab.style.cssText = 'flex: 1; padding: 10px; background: var(--bg-input); color: var(--text-primary); border: 1px solid var(--border); border-radius: 8px; cursor: pointer; transition: all 0.2s;';
    ...
    fileInput.style.cssText = 'padding: 12px; background: var(--bg-input); border: 1px solid var(--border); border-radius: 8px; color: var(--text-primary); width: 100%; margin-bottom: 16px;';
...
function createYouTubeInputSection() {
    ...
    urlInput.style.cssText = 'flex: 1; padding: 12px; background: var(--bg-input); border: 1px solid var(--border); border-radius: 8px; color: var(--text-primary);';
...
function createYouTubePitchControl() {
    ...
    control.style.cssText = 'display: none; margin-top: 16px; padding: 16px; background: var(--bg-input); border-radius: 8px;';
    ...
    description.style.cssText = 'margin: 0; font-size: 13px; color: var(--text-secondary); line-height: 1.4;';
    description.innerHTML = 'Allows independent pitch control without affecting playback speed. <span style="color: var(--warning);">Note: This will open a tab audio capture dialog.</span>';
    control.appendChild(description);
    
    const indicator = document.createElement('div');
    indicator.id = 'pitchActiveIndicator';
    indicator.style.cssText = 'display: none; margin-top: 12px; padding: 8px; background: var(--success-bg); border-radius: 6px; color: var(--success); font-size: 13px; text-align: center;';
...
function createAudioControlsSection() {
    ...
    infoText.style.cssText = 'background: rgba(99, 102, 241, 0.1); padding: 16px; border-radius: 8px; border-left: 4px solid var(--primary);';
    const infoParagraph = document.createElement('p');
    infoParagraph.style.cssText = 'margin: 0; color: var(--text-secondary);';
    infoParagraph.textContent = '💡 Tip: Use the "Save Current Session" button to save your loop points and settings';
...
function createWaveformContainer() {
    ...
    container.style.cssText = 'position: relative; width: 100%; height: 150px; background: var(--bg-input); border-radius: 8px; overflow: hidden; margin-bottom: 20px;';
    
    // Loading state
    const loadingState = document.createElement('div');
    loadingState.id = 'waveformLoadingState';
    loadingState.style.cssText = 'position: absolute; top: 0; left: 0; width: 100%; height: 100%; display: flex; align-items: center; justify-content: center; background: var(--bg-input); z-index: 10;';
    
    const loadingContent = document.createElement('div');
    loadingContent.style.textAlign = 'center';
    
    const spinner = document.createElement('div');
    spinner.style.cssText = 'width: 40px; height: 40px; border: 3px solid var(--border); border-top-color: var(--primary); border-radius: 50%; animation: spin 1s linear infinite; margin: 0 auto 10px;';
    ...
    canvas.style.cssText = 'width: 100%; height: 100%; display: block; opacity: 0; transition: opacity 0.3s ease;';
    ...
    loopRegion.style.cssText = 'position: absolute; top: 0; height: 100%; background: rgba(99, 102, 241, 0.2); pointer-events: none; display: none;';
...
function createPlaybackControls() {
    ...
    controls.style.cssText = 'display: flex; align-items: center; gap: 16px; margin-bottom: 24px; justify-content: center;';
...
function createLoopControls() {
    ...
    mainRow.style.cssText = 'display: flex; gap: 8px; align-items: center; margin-bottom: 12px; flex-wrap: wrap;';
...
function createProgressionControls() {
    ...
    controls.style.cssText = 'display: none; align-items: center; gap: 6px; font-size: 12px; margin-top: 12px;';
    ...
    incrementInput.style.cssText = 'width: 45px; padding: 3px 6px; background: var(--bg-dark); border: 1px solid var(--border); border-radius: 4px; font-size: 12px;';
    ...
    intervalInput.style.cssText = 'width: 35px; padding: 3px 6px; background: var(--bg-dark); border: 1px solid var(--border); border-radius: 4px; font-size: 12px;';
...
function createCompactAudioControls() {
    ...
    controls.style.cssText = 'background: rgba(255, 255, 255, 0.05); backdrop-filter: blur(10px); padding: 20px; border-radius: 12px; margin-bottom: 20px; border: 1px solid rgba(255, 255, 255, 0.1);';
...
function createSpeedControl() {
    ...
    const label = document.createElement('label');
    label.style.cssText = 'display: block; margin-bottom: 8px; font-size: 14px; color: var(--text-secondary);';
    label.innerHTML = 'Speed: <span id="speedValue" style="color: var(--primary); font-weight: 600;">100%</span>';
    ...
    slider.style.cssText = 'width: 100%; height: 8px; background: linear-gradient(to right, #374151 0%, #6366f1 50%, #374151 100%); border-radius: 4px; outline: none; -webkit-appearance: none; -moz-appearance: none; appearance: none;';
    control.appendChild(slider);
    
    const markers = document.createElement('div');
    markers.style.cssText = 'display: flex; justify-content: space-between; margin-top: 4px; font-size: 11px; color: var(--text-muted);';
...
function createPitchControl() {
    ...
    const label = document.createElement('label');
    label.style.cssText = 'display: block; margin-bottom: 8px; font-size: 14px; color: var(--text-secondary);';
    label.innerHTML = 'Pitch: <span id="pitchValue" style="color: var(--primary); font-weight: 600;">0</span>';
    control.appendChild(label);
...
function createVolumeControl() {
    ...
    const label = document.createElement('label');
    label.style.cssText = 'display: block; margin-bottom: 8px; font-size: 14px; color: var(--text-secondary);';
    label.innerHTML = 'Volume: <span id="volumeValue" style="color: var(--primary); font-weight: 600;">100%</span>';
    control.appendChild(label);
    ...
    slider.value = '100';
    slider.className = 'slider';
    slider.style.cssText = 'flex: 1; height: 8px; background: linear-gradient(to right, #374151 0%, #6366f1 100%); border-radius: 4px; outline: none; -webkit-appearance: none; -moz-appearance: none; appearance: none;';
