...
<div class="saved-loops-section" style="background: rgba(255, 255, 255, 0.05); backdrop-filter: blur(10px); padding: 16px; border-radius: 12px; margin-bottom: 20px; border: 1px solid rgba(255, 255, 255, 0.1);">
    ...
    attachEventListeners() {
        ...
        if (loopControlsSection) {
            loopControlsSection.addEventListener('click', (e) => {
                if (e.target && e.target.id === 'saveSessionBtn') {
        ...
        if (volumeSlider) {
            volumeSlider.addEventListener('input', (e) => {
                ...
                if (this.masterGain) {
                    ...
                    this.masterGain.gain.value = normalizedValue * normalizedValue;
                }
            });
        }
    ...
    async handleFileSelect(event) {
        ...
        try {
            ...
            this.grainPlayer = new Tone.GrainPlayer({
                ...
                loop: false,
                playbackRate: 1.0,
                grainSize: 0.05,    // Small grains for best quality
                overlap: 0.75,      // High overlap for smoothest pitch shifting
                reverse: false,
                volume: 0,          // Start at 0dB (unity gain)
                onload: () => {
                    try {
                        ...
                        // Connect audio chain with high-quality pitch shifter
                        // Use a simpler approach - let GrainPlayer handle tempo, use pitch shifter for pitch only
                        this.grainPlayer.detune = 0; // Reset detune
    ...
    async togglePlayPause() {
        ...
        if (this.isPlaying) {
            ...
            // Start playback without fade-in
    ...
    setSpeed(speed) {
        ...
        if (this.isYouTubeMode && this.youtubePlayer) {
            ...
            // Compensate for pitch change due to playback rate
            // When speed increases, pitch goes up, so we need to shift down
            const pitchCompensation = -12 * Math.log2(this.playbackRate);

            // Apply both user pitch shift and compensation
            this.applyHighQualityPitchShift(this.pitchShiftAmount + pitchCompensation);
        }
    }
    ...
    setPitch(pitch) {
        ...
        if (this.isYouTubeMode) {
            ...
            // For regular audio files, implement better pitch shifting
            // Also compensate for playback rate pitch change
            const pitchCompensation = -12 * Math.log2(this.playbackRate);
            this.applyHighQualityPitchShift(pitch + pitchCompensation);
        }
    }
    ...
    setQualityMode(mode = 'high') {
        const qualitySettings = {
            low: {
                grainSize: 0.2,      // Larger grains for lower CPU
                overlap: 0.25        // Less overlap
            },
            medium: {
                grainSize: 0.1,      // Medium grain size
                overlap: 0.5         // Good overlap
            },
            high: {
                grainSize: 0.05,     // Small grains for best quality
                overlap: 0.75        // High overlap for smoothest sound
            }
        };
