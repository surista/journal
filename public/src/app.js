...
class App {
    ...
    async init() {
        ...
        try {
            ...
            // Note: StorageService will be passed later after auth check
            ...
            if (!authModule.AuthService) {
                console.error(
                    'AuthService not found in module. Available exports:',
                    Object.keys(authModule)
                );
            ...
            // Update theme service with storage service for cloud sync
            if (this.themeService) {
                this.themeService.storageService = this.storageService;
                // Re-initialize theme to load from cloud if available
    ...
    async loadDashboardPage() {
        try {
            ...
            const dashboard = new dashboardModule.DashboardPage(
                this.storageService,
                this.authService
            );
    ...
    showUpdateNotification() {
        ...
        updateButton.style.cssText =
            'padding: 0.5rem 1rem; background: var(--primary); color: white; border: none; border-radius: 6px; cursor: pointer; font-weight: 500;';
    ...
    showError(message, error = null) {
        ...
        errorContainer.style.cssText =
            'min-height: 100vh; display: flex; align-items: center; justify-content: center; background: var(--bg-dark); padding: 20px;';

        const errorCard = document.createElement('div');
        errorCard.style.cssText =
            'background: var(--bg-card); padding: 2rem; border-radius: 12px; border: 1px solid var(--danger); max-width: 600px; width: 100%;';
        ...
        if (error) {
            ...
            pre.style.cssText =
                'background: var(--bg-input); padding: 1rem; border-radius: 6px; overflow-x: auto; margin-top: 1rem; color: var(--text-primary); font-size: 0.875rem;';
        ...
        reloadButton.style.cssText =
            'padding: 0.75rem 1.5rem; background: var(--primary); color: white; border: none; border-radius: 6px; cursor: pointer;';
        reloadButton.addEventListener('click', () => location.reload());

        const clearCacheButton = document.createElement('button');
        clearCacheButton.textContent = 'Clear Cache & Reload';
        clearCacheButton.style.cssText =
            'padding: 0.75rem 1.5rem; background: var(--danger); color: white; border: none; border-radius: 6px; cursor: pointer;';
